...
| `PRIMARY_KEY`  | id                | Primary key for the song                           |                     |          |
...
| `FOREIGN KEY`  | user_id            | `auth.users.id` (`ON UPDATE CASCADE`, `ON DELETE CASCADE`) |                     |          |
| `FOREIGN KEY`  | song_id            | `songs.id`      (`ON UPDATE CASCADE`, `ON DELETE CASCADE`) |                     |          |
...
| `FOREIGN KEY`  | playlist_id              | `playlists.id`  (`ON UPDATE CASCADE`, `ON DELETE CASCADE`)  |                     |          |
| `FOREIGN KEY`  | song_id                  | `songs.id`      (`ON UPDATE CASCADE`, `ON DELETE CASCADE`)  |                     |          |
| `FOREIGN KEY`  | added_by                 | `auth.users.id` (`ON UPDATE CASCADE`, `ON DELETE SET NULL`) |                     |          |
...
## `chats` Table
This table contains information about user chat with the bot. It stores messages exchanged between users and the bot, including the user ID, message content, and timestamps.

| Field Name     | Data Type           | Description                                                         | Default Value       | Nullable |
|----------------|---------------------|---------------------------------------------------------------------|---------------------|----------|
| `id`           | Integer             | Unique identifier for the chat message                              |                     | No       |
| `user_id`      | UUID                | Unique identifier for the user                                      |                     | No       |
| `message`      | Text                | Content of the chat message                                         | ""                  | No       |
| `is_bot`       | Boolean             | Indicates if the message is from the bot (true) or the user (false) | FALSE               | No       |
| `created_at`   | Timestamp           | Timestamp when the message was sent                                 | `now()`             | No       |
| `FOREIGN KEY`  | `user_id`           | `auth.users.id` (`ON UPDATE CASCADE`, `ON DELETE CASCADE`)          |                     |          |